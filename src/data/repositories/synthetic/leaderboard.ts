...
export function buildLeaderboardProvenance(
  dataset: SyntheticDataset,
  slug: string,
  viewer: ViewerContext,
  ...
  // Only the currently active, unreversed approved original per opportunity.
...
export const syntheticLeaderboardRepository: LeaderboardRepository = {
  async list(viewer: ViewerContext): Promise<LeaderboardRow[]> {
    return buildLeaderboardRows(loadSyntheticDataset(), viewer);
  },

  async getProvenance(slug: string, viewer: ViewerContext): Promise<LeaderboardProvenance | null> {
    return buildLeaderboardProvenance(loadSyntheticDataset(), slug, viewer);
  },
};
